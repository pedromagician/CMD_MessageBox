...
static inline CBT_CREATEWND* AsCreateStruct(int code, LPARAM lParam)
{
...
LRESULT CALLBACK MessageBoxPos::SetPosition(int _code, WPARAM _wParam, LPARAM _lParam)
{
	...
	case _RIGHT_CENTER: {
		if (auto* s = AsCreateStruct(_code, _lParam)) {
...
int MessageBoxPos::MessageBox(HWND _hWnd, LPCTSTR _text, LPCTSTR _caption, UINT _type)
{
	...
	int result = 0;
	if (hHook1 && hHook2) {
		result = ::MessageBox(_hWnd, _text, _caption, _type);
		if (hHook1) UnhookWindowsHookEx(hHook1);
		if (hHook2) UnhookWindowsHookEx(hHook2);
	}
	else {
		wcout << _T("Error - Problem creating a hook") << endl;
	}
